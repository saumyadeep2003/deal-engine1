# Tianzheng Biotechnology Inc — intelligence brief

Generated: 2026-08-05T10:34:38Z | trigger: auto_threshold | recommendation: **Deep Dive**

## Funding history

- No round observed in free sources. Full history — (requires PitchBook).

## Cap table quality

- Tier 1: 0, Tier 2: 0, Tier 3: 0 [computed] (observed investments × config tier list; full cap table — requires PitchBook)

## Team & hiring signals

- Headcount / 6-month growth: — (requires Coresignal)

## Product traction

- 2026-07-14 filing: Tianzheng Biotechnology Inc [S:10]

## Thesis fit (computed)

- 100th percentile of 6 in cohort biotech_health|unknown — LOW CONFIDENCE (cohort < 20) [computed]
- Feature vector stored (scores.features_json, model=computed-v1, prompt=score-p1) [computed]

## Investor & operator commentary

- None captured yet in free sources (HN/Reddit); X, Blind, podcasts, Substack threads — (require licenses).
## Judgment (model-generated — labelled, distinct from observed data)

- Founder quality: [STUB: no API key — judgment unavailable]
- Moat / defensibility: [STUB: no API key — judgment unavailable]
- TAM: [STUB: no API key — judgment unavailable]
- Meta-thesis fit: [STUB: no API key — judgment unavailable]
- Exit horizon: [STUB: no API key — judgment unavailable]

### Thesis narrative
[STUB: no API key — judgment unavailable]

## Comparable companies
- Vitanova Biomedical Inc. (market rank 2 in cohort) [computed]
- Mentat Biotechnology, Corp. (market rank 3 in cohort) [computed]
- Cyrus Biotechnology, Inc. (market rank 4 in cohort) [computed]
- Portal Biotechnologies, Inc. (market rank 5 in cohort) [computed]
- Los Angeles Project Inc (market rank 6 in cohort) [computed]

## Recommendation
**Deep Dive** [computed] (percentile thresholds in config/thesis.yaml; partner override wins)
