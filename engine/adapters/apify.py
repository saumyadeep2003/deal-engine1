...
class ApifyAdapter(BaseAdapter):
    ...
    def probe(self) -> dict:
        """Ask Apify who we are. The cheapest call that proves the token is real:
        it costs no Actor run, and it separates "token rejected" from "cannot
        reach Apify" — different problems with different fixes. Running an Actor
        to test the connection would bill the user for a diagnostic."""
        return self_test()
