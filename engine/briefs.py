...
def validate_brief(md: str, company_id: int | None = None) -> list[str]:
    """Violations that block publication.

    Two layers, and the second one is the one that matters. The first checks that
    every numeric claim carries a citation MARKER — shape only. On its own that is
    close to useless: an invented "[S:99999]" satisfies it perfectly, which is
    exactly how a fabricated claim can look better sourced than a real one.

    So when the company is known, every citation is also resolved against the
    database: the signal must exist and must belong to THIS company. Borrowing a
    real id from another company is the failure mode a shape check can never see.
    """
    violations = []
    ...
    if company_id is not None:
        violations += [f"unresolvable citation: {b}"
                       for b in gatekeeper.audit_citations(md, company_id)]
    return violations
...
def _judgment_sections(company_id: int, judged: dict | None,
                       removals: list | None = None) -> str:
    """The only section a model writes. Everything above it is assembled from
    rows, so this is where invention can enter — and therefore the section that
    goes through the gatekeeper before a word of it is rendered."""
    if judged:
        judged, removed = gatekeeper.verify_judgement(judged, company_id)
        if removed and removals is not None:
            removals.extend(removed)
    out = ["\n\n## What the AI makes of it\n",
           "*Model-written judgement — labelled on purpose, and separate from the observed facts above. Numbers here are opinions, not measurements. Every sentence in this section was checked against the stored evidence; anything that could not be traced was removed and marked.*\n"]
    if not judged:
    ...
    def rated(label: str, score_key: str, reason_key: str) -> str:
        """A score with no surviving justification is not shown as a score. The
        gatekeeper nulls it when its entire reasoning was untraceable, and printing
        'None/10' or a bare number next to a removal marker would recreate exactly
        the confident-looking emptiness this is meant to prevent."""
        score, reason = judged.get(score_key), judged.get(reason_key)
        if score in (None, ""):
            why = ("its justification could not be traced to any stored evidence"
                   if reason and gatekeeper.REMOVED_MARKER in str(reason)
                   else "the evidence did not support a score")
            return f"- {label}: not assessed — {why}"
        return f"- {label}: {score}/10 — {reason or 'n/a'}"

    out.append(rated("Founder quality", "founder_quality", "founder_reasoning"))
    out.append(rated("Moat", "moat", "moat_reasoning"))
    if tam.get("value_usd"):
        # marked as an estimate in the text itself, not just in a bracket: a reader
        # skimming must not be able to mistake this for a measured figure
        out.append(f"- TAM (model estimate, not measured): ${tam['value_usd'] / 1e9:.1f}B"
                   f" [S:model-estimate] — confidence: {tam.get('confidence')}"
                   f" — assumptions: {'; '.join(tam.get('assumptions') or [])}")
    else:
        out.append("- TAM: null (insufficient context — honest null beats an invented number)")
    out.append(rated("Meta-thesis fit", "meta_thesis_fit", "meta_thesis_reasoning"))
    out.append(f"- Exit horizon: {judged.get('exit_horizon_years') or 'null'} years")
    out.append(f"\n### Thesis narrative\n{judged.get('thesis_narrative') or 'null'}")
    return "\n".join(out)


def _removal_category(removal: dict) -> str:
    """A partner does not need the failing sentence on the page; they need to know
    which KIND of thing the model reached for, because the three mean different
    problems: bad citations mean the model is inventing provenance, bad figures
    mean it is inventing measurements, bad names mean it is filling gaps from
    memory. Each points at a different fix."""
    reasons = " ".join(removal.get("reasons") or [])
    if "cites" in reasons:
        return "citation pointing at no stored signal"
    if "matches no stored value" in reasons:
        return "figure matching no stored value"
    if "appears nowhere" in reasons:
        return "party absent from this company's evidence"
    return "untraceable claim"
...
def generate_brief(company_id: int, trigger: str = "on_demand",
    ...
    if removals:
        # Say it on the page. A brief that quietly lost a sentence reads as
        # complete; a brief that says what was stopped lets a partner judge both
        # the company and how much the model is reaching.
        #
        # The footer names CATEGORIES, never the removed text. Quoting the
        # offending sentence back — "states $42M, which matches no stored value" —
        # puts the invented figure and its fake citation straight back on the page,
        # where the citation validator then (correctly) fails the whole brief. The
        # full text and reasons live in gatekeeper_events, which is where an
        # auditor should be reading them anyway.
        md += ("\n\n---\n\n*Gatekeeper: "
               f"{len(removals)} model-written claim(s) [computed] were removed from the"
               " judgement section and marked in place above — "
               + ", ".join(sorted({_removal_category(r) for r in removals}))
               + ". The exact wording and reason for each is stored in"
                 " `gatekeeper_events` for audit.*\n")
        gatekeeper.record(company_id, "brief", removals, ref=trigger)

    violations = validate_brief(md, company_id)
    if violations and judged:
        ...
        violations2 = validate_brief(md2, company_id)
        if not violations2:
            md, violations = md2, []
        else:
